...
function parseDetectionResult(stdout) {
...
// src/descriptor.ts
var LABEL_PREFIX = "data:";
var DESCRIPTOR_RE = /^[a-z0-9](?:[a-z0-9._-]*[a-z0-9])?$/;
...
function descriptorFromLabel(label) {
...
async function dedupe(port, descriptor) {
...
var PROVENANCE_SCHEMA = "continuous-research/provenance@v1";
var HASH_RE = /^[a-z0-9]+:[0-9a-f]+$/;
...
function buildProvenanceStub(input) {
...
function serializeProvenanceStub(stub) {
  return `${JSON.stringify(stub, null, 2)}
`;
}
function provenanceFile(stub) {
  return { path: provenancePathFor(stub.descriptor), content: serializeProvenanceStub(stub) };
}

// src/decline.ts
function requireIsoDate2(value) {
  if (Number.isNaN(Date.parse(value))) {
    throw new Error(`Decline declinedAt must be an ISO-8601 date, got ${JSON.stringify(value)}`);
  }
  return value;
}
function renderDeclineRecord(input) {
  const descriptor = assertDescriptor(input.descriptor);
  const declinedAt = requireIsoDate2(input.declinedAt);
  const reason = input.reason.trim();
  const lines = [];
  lines.push("---");
  lines.push(`descriptor: "${descriptor}"`);
  lines.push(`declined_at: "${declinedAt}"`);
  if (input.prNumber !== void 0) lines.push(`data_pr: ${input.prNumber}`);
  if (input.declinedBy !== void 0) lines.push(`declined_by: "${input.declinedBy}"`);
  lines.push("---");
  lines.push("");
...
async function proposeDataPR(port, input) {
...
async function recordDecline(port, input) {
  const base = await port.defaultBranch();
...
async function runRecordDecline(deps) {
...
function labelNamesOf(labels) {
  return labels.map((l) => typeof l === "string" ? l : l.name ?? "").filter((name) => name !== "");
}
function pullStateOf(issueState, mergedAt) {
  if (issueState === "open") return "open";
  return mergedAt != null ? "merged" : "closed_unmerged";
}
function mapIssueToPullRequest(raw) {
...
var OctokitGitHubPort = class {
  ...
  async listPullRequestsByLabel(label) {
  ...
  async provenanceStubExists(descriptor) {
    try {
  ...
  async putFile(input) {
    await this.octokit.rest.repos.createOrUpdateFileContents({
      ...
      content: Buffer3.from(input.content, "utf8").toString("base64"),
      branch: input.branch
    });
  }
...
var execSensor = async (command) => {
  const { stdout } = await execAsync(command, { maxBuffer: 16 * 1024 * 1024 });
  return stdout;
};
var readArtifact = (path) => readFile(path, "utf8");
function parseRepository(slug) {
...
function createGitHubPortFromEnv(env) {
  ...
  return new OctokitGitHubPort({ octokit: new Octokit2({ auth: token }), owner, repo });
}
...
function firstDataDescriptor(labels) {
...
function senderLogin(sender) {
  if (typeof sender === "object" && sender !== null && "login" in sender) {
    return typeof sender.login === "string" ? sender.login : void 0;
  }
  return void 0;
}
function extractDeclineFromEvent(event) {
  ...
  const declinedAt = typeof p.closed_at === "string" ? p.closed_at : (/* @__PURE__ */ new Date()).toISOString();
  return {
    descriptor,
    prNumber: p.number,
    declinedAt,
    declinedBy: senderLogin(event.sender)
  };
}

// src/cli.ts
async function cmdSense() {
  const config = parseConfig(await readFile2(".research/config.json", "utf8"));
...
async function cmdRecordDecline() {
  const eventPath = process.env.GITHUB_EVENT_PATH;
  if (!eventPath) throw new Error("GITHUB_EVENT_PATH is required");
  const event = JSON.parse(await readFile2(eventPath, "utf8"));
...
async function main() {
  ...
  const handler2 = COMMANDS[command];
  if (!handler2) {
